  ...
  <div class="management-page-background">
    <div class="container-management">
      ...
      <div class="action-bar">
        ...
        <button class="btn add-btn" @click="showAddDialog">新增攻略</button>
      </div>
      <!-- 数据表格 -->
          ...
          <table class="data-table">
            ...
            <tbody>
              <tr v-for="card in filteredCards" :key="card.id">
                ...
                <td>{{ card.id }}</td>
                <td>{{ card.title.substring(0, 3) }}...</td>
                <td>
                  <img :src="card.image" alt="攻略图片" style="width: 35px; height: 35px;" @click="triggerFileInput(card)">
                </td>
                <td>{{ card.description.substring(0, 10) }}...</td>
                <td>{{ card.badgeText }}</td>
                <td>{{ card.score }}</td>
                <td>{{ card.likes }}</td>
                <td>{{ card.comments }}</td>
                <td>{{ card.createAt }}</td>
        ...
        <div class="dialog" @click.stop>
          ...
          <form @submit.prevent="submitForm" class="form-container">
            <div class="form-row">
              ...
              <div class="form-group">
                <label>攻略徽章:</label>
                <input v-model="formData.badgeText" required />
              </div>
            </div>
            <!-- 创建修改时间 -->
...
const columns = [
  ...
  { key: 'id', title: '攻略ID' },
  { key: 'title', title: '攻略标题' },
  { key: 'image', title: '攻略图片' },
  { key: 'description', title: '攻略副标题' },
  { key: 'badgeText', title: '攻略徽章' },
  { key: 'score', title: '攻略评分' },
  { key: 'likes', title: '点赞数' },
...
const fetchScenic = async () => {
  try {
    ...
    const response = await request.get('/api/public/travelcard', { params });
